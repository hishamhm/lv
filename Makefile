...
graph.avi: graph-0000001.png
	rm -f graph.avi && ffmpeg -f image2 -framerate 50 -i graph-%07d.png graph.avi

graph-0000001.png: graph-0000001.dot
	n=$$(ls graph-*.dot | wc -l); for i in $$(seq -f "%07.0f" $$n); do echo $$i/$$n; dot -Tpng graph-$$i.dot > graph-$$i.png; done
